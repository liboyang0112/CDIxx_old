...
__global__ void takeMod2Diff(complexFormat* a, Real* b, Real *output, spt *bs){
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
  if(x >= cuda_row || y >= cuda_column) return;
  int index = x*cuda_column + y;
  output[index] = ((bs&&bs->isInside(x,y))|| b[index]>0.95)?0:(b[index]-pow(a[index].x,2)-pow(a[index].y,2));
}
...
__global__ void applyMod(complexFormat* source, Real* target, ImageMask *bs = 0, bool loose=0, int iter = 0, int noiseLevel = 0){
  assert(source!=0);
  assert(target!=0);
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void applySupport(complexFormat *gkp1, complexFormat *gkprime, Real* objMod, ImageMask *spt, int iter = 0, Real fresnelFactor = 0){
  ...
  if(iter < 200) ApplyRAARSupport(inside,gkp1data,gkprimedata,cuda_beta_HIO);
...
Mat* phaseRetrieve( experimentConfig &setups, Mat* targetfft, Mat* &gkp1, Mat *cache = 0, Mat* fftresult = 0 ){
  ...
  complexFormat *cuda_fftresult, *cuda_gkprime, *cuda_gkp1, *cuda_pmpsg;
  Real *cuda_targetfft, *cuda_diff;
  Real *cuda_objMod;
  ImageMask *cuda_spt;
  gpuErrchk(cudaMalloc((void**)&cuda_fftresult, sz));
  cudaMalloc((void**)&cuda_targetfft, sz/2);
  ...
  cudaMemcpy(cuda_fftresult, targetfft->data, sz, cudaMemcpyHostToDevice);
  setups.propagate(cuda_fftresult, cuda_gkp1,0);
  getMod2<<<numBlocks,threadsPerBlock>>>(cuda_targetfft, cuda_fftresult);
  ...
  for(int iter = 0; iter < setups.nIter; iter++){
    ...
    if(iter%100==0) {
      ...
      if(iter>0){
        takeMod2Diff<<<numBlocks,threadsPerBlock>>>(cuda_fftresult,cuda_targetfft, cuda_diff, useBS? cir.cuda:0);
        ...
        cudaMemcpy(objMod.data, cuda_diff, sz/2, cudaMemcpyDeviceToHost);
        convertFromComplexToInteger<Real>(&objMod, cache,MOD,1, 1.,"smoothed", 1);
        imwrite("smoothed"+iterstr+".png",*cache);
      }
    }


    applyMod<<<numBlocks,threadsPerBlock>>>(cuda_fftresult,cuda_diff, useBS? cir.cuda:0, !setups.reconAC || iter > 1000,iter, setups.noiseLevel);
    ...
    if((iter > 0 && iter < 200) && iter%20==0 && useShrinkMap){
      ...
      re.threshold = findMax(cuda_objMod, row*column)*setups.shrinkThreshold;
      cudaMemcpy(cuda_spt, &re, sizeof(ImageMask), cudaMemcpyHostToDevice);
      re.cpyToGM();
      if(gaussianSigma>1.5) {
        gaussianSigma*=0.99;
        //setups.shrinkThreshold*=0.991;
      }
      delete maskKernel;
      if(iter%100==0&&setups.saveIter){
  ...
  applyMod<<<numBlocks,threadsPerBlock>>>(cuda_fftresult,cuda_targetfft,useBS?cir.cuda:0,1,setups.nIter, setups.noiseLevel);
  cudaMemcpy(gkp1->data, cuda_gkp1, sz, cudaMemcpyDeviceToHost);
  cudaMemcpy(fftresult->data, cuda_fftresult, sz, cudaMemcpyDeviceToHost);
  cudaFree(cuda_gkp1);
...
int main(int argc, char** argv )
{
  ...
  convertFromComplexToInteger(targetfft, cache, MOD2,1,1,"log Pattern MOD2",1);
