...
void addNoise(Mat& src, Mat& dest, double sigma, double sprate = 0.0)
{
    ...
    if (src.channels() == 1)
    {
        ...
        if (sprate != 0)
            addNoiseSoltPepperMono(dest, dest, sprate); //然后添加椒盐噪点
        return;
    }
    else
    {
...
{
    const double noise_sigma = 50.0;
    Mat src = imread("./src/src.png", IMREAD_UNCHANGED);
    Mat snoise;
    Mat dest(src.rows,src.cols,CV_16UC1);
    addNoise(src, snoise, noise_sigma);
    imwrite("./src/noise.png",snoise);
    //高斯滤波
    int64 pre = getTickCount();
    GaussianBlur(src, dest, Size(7, 7), 5);
   imwrite("./src/gaussian.png", dest);
   //中值滤波
    pre = getTickCount();
    medianBlur(src, dest, 3);
    imwrite("./src/median.png", dest);
   //双边滤波
    pre = getTickCount();
    bilateralFilter(src, dest, 7, 35, 5);
    imwrite("./src/bilateral.png", dest);
    //非局部均值
    pre = getTickCount();
    nonlocalMeansFilter(src, dest, 7, 10, noise_sigma, noise_sigma);
    cout << "nonlocal: " << calcPSNR(src, dest) << endl
        << endl;
    imwrite("./src/nonlocal1.png", dest);
    system("open ./src/nonlocal1.png");
    return 0;
}
