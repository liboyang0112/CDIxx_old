...
static const int rcolor = pow(2,8);
...
bool rectSupport(int x, int y, rect *re){
  if(x >= re->startx && x <= re->endx && y >= re->starty && y <= re->endy) return true;
  else return false;
}
...
int main(int argc, char** argv )
{
    ...
    fftresult = fftw(gkp1,row,column,fftresult,1);
    pmpsg = fftw(gkp1,row,column,pmpsg,1);
    rect re;
    ...
    fepF.open("epsilonF.txt",ios::out);
